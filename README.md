...
The driver provides direct access to all AXP2101 registers through the low-level API via `axp.ll`. This API is automatically generated from [`device.ddsl`](device.ddsl) and provides type-safe access to all register fields.
...
- Field: `dcdc1_enable` → `set_dcdc_1_enable()` / `dcdc_1_enable()` (digits are split into their own word)
...
The AXP2101 register map is defined in [`device.ddsl`](device.ddsl), which `device-driver` uses to generate Rust code. This file specifies:
